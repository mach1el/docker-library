...
if ($action=="search")
{
  ...
  // Incoming filters (may be empty)
  $dialplan_id = isset($_POST['dialplan_id']) ? trim($_POST['dialplan_id']) : "";
  $match_exp   = isset($_POST['match_exp'])   ? trim($_POST['match_exp'])   : "";
  $repl_exp    = isset($_POST['repl_exp'])    ? trim($_POST['repl_exp'])    : "";
  $attrs       = isset($_POST['attrs'])       ? trim($_POST['attrs'])       : "";

  // Whenever we search, go back to page 1
  $_SESSION[$current_page] = 1;

  if ($show_all == "Show All") {
    // Clear all filters
    $_SESSION['dialplan_id']        = "";
    $_SESSION['dialplan_match_exp'] = "";
    $_SESSION['dialplan_repl_exp']  = "";
    $_SESSION['dialplan_attrs']     = "";
  } else if ($search == "Search") {
    // Store filters for main page
    $_SESSION['dialplan_id']        = $dialplan_id;
    $_SESSION['dialplan_match_exp'] = $match_exp;
    $_SESSION['dialplan_repl_exp']  = $repl_exp;
    $_SESSION['dialplan_attrs']     = $attrs;
  }
}
