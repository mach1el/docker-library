...
// filters coming from search form
$search_dpid      = isset($_SESSION['dialplan_id'])        ? $_SESSION['dialplan_id']        : "";
$search_match_exp = isset($_SESSION['dialplan_match_exp']) ? $_SESSION['dialplan_match_exp'] : "";
$search_repl_exp  = isset($_SESSION['dialplan_repl_exp'])  ? $_SESSION['dialplan_repl_exp']  : "";
$search_attrs     = isset($_SESSION['dialplan_attrs'])     ? $_SESSION['dialplan_attrs']     : "";
...
// filter by dialplan id (group)
if ($search_dpid != "") {
  $conditions[] = "dpid = ?";
  $sql_vals[]   = $search_dpid;
}

// filter by matching regular expression (match_exp)
if ($search_match_exp != "") {
  $conditions[] = "match_exp LIKE ?";
  $sql_vals[]   = "%" . $search_match_exp . "%";
}

// filter by replacement expression (repl_exp)
if ($search_repl_exp != "") {
  $conditions[] = "repl_exp LIKE ?";
  $sql_vals[]   = "%" . $search_repl_exp . "%";
}

// filter by attributes (attrs)
if ($search_attrs != "") {
  $conditions[] = "attrs LIKE ?";
  $sql_vals[]   = "%" . $search_attrs . "%";
}

// build final WHERE fragment if any filters were provided
if (count($conditions)) {
  $sql_search = implode(" AND ", $conditions);
}
...
<tr>
  <td class="searchRecord">Match Expression :</td>
  <td class="searchRecord" width="200">
    <input type="text" name="match_exp" value="<?=$search_match_exp?>" class="searchInput">
  </td>
</tr>

<tr>
  <td class="searchRecord">Replacement Expression :</td>
  <td class="searchRecord" width="200">
    <input type="text" name="repl_exp" value="<?=$search_repl_exp?>" class="searchInput">
  </td>
</tr>
...
<tr>
  ...
  <td class="searchRecord" width="200">
    <input type="text" name="attrs" value="<?=$search_attrs?>" class="searchInput">
  </td>
</tr>
